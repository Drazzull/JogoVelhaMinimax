...
public class MiniMax {
        ...
        for(int i = 0; i < tam; i++){
            for(int j = 0; j < tam; j++){
                if(tabuleiro.getTabuleiro()[i][j] == '-'){
                    valores[i][j] = 0;
                } else if(tabuleiro.getTabuleiro()[i][j] == 'X'){
                    valores[i][j] = 1;
                } else{
                    valores[i][j] = -1;
                }
    ...
    public int valorMax(int[][] tab, boolean prim, int prof){
        profundidade++;
        ...
        for(Sucessor s : gerarSucessores(tab, 1)){
            v = Math.max(v, valorMin(s.tabuleiro, prof));
            s.utilidade = v;
            if(prim){
                sucessores.add(s);
            }
    ...
    public int valorMin(int[][] tab, int prof){
        profundidade++;
        if(testeTerminal(tab))
            return utilidade(tab);
        ...
        for(Sucessor s : gerarSucessores(tab, -1)){
            v = Math.min(v, valorMax(s.tabuleiro, false, prof));
            s.utilidade = v;
        }
        return v;
    }
